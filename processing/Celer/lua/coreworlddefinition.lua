...
local function can_be_added_to_unit_group(name)
...
elseif level_id == 'bex' or level_id == 'skm_bex' then
	...
	function WorldDefinition:make_unit(data, ...)
		...
		if junk[unit_id] then
			return
		elseif not can_be_added_to_unit_group(name) then
		...
		if unit_id == 900705 and name == 'units/pd2_dlc_arena/props/are_prop_office_lamp_ceiling/are_prop_office_lamp_ceiling_short' then
			unit_id = 990705 -- conflicting IDs... *slow clap*
		end
...
elseif level_id == 'big' or level_id == 'skm_big2' then
	...
	function WorldDefinition:make_unit(data, ...)
		local name = data.name
		if not can_be_added_to_unit_group(name) or name == 'units/payday2/props/bnk_prop_security_keybox/bnk_prop_security_keybox' then
			return cel_original_worlddefinition_makeunit(self, data, ...)
		end
...
elseif level_id == 'bph' then
	...
	function WorldDefinition:make_unit(data, ...)
		local name = data.name
		if not can_be_added_to_unit_group(name) then
			return cel_original_worlddefinition_makeunit(self, data, ...)
		end
...
elseif level_id == 'dah' then
	...
	Celer:spawn_occluder('16x16', Vector3(-4260, -1770, 760), Rotation(-45, 90, 0))
	Celer:spawn_occluder('16x16', Vector3(-4260, -1770, 760), Rotation(-45, 90, 0), true)
...
elseif level_id == 'des' then
	...
	function WorldDefinition:make_unit(data, ...)
		...
		or not can_be_added_to_unit_group(name)
...
elseif level_id == 'mus' or level_id == 'skm_mus' then
	...
	Celer:spawn_occluder('4x8', Vector3(-1050, -1520, -400), Rotation(90, 0, 0))
...
elseif level_id == 'peta' then
	...
	Celer:spawn_occluder('24x16', Vector3(3306, 2900, 875), Rotation(0, 0, 180))
...
elseif level_id == 'sah' then
	...
	function WorldDefinition:make_unit(data, ...)
		local unit_id = data.unit_id
		local name = data.name
		if not can_be_added_to_unit_group(name) or name == 'units/pd2_dlc_sah/props/sah_interactable_hackbox/sah_interactable_hackbox' then
			return cel_original_worlddefinition_makeunit(self, data, ...)
		end
...
elseif level_id == 'vit' then
	...
	Celer:spawn_occluder('16x8', Vector3(900, 3450, 200), Rotation(0, 0, -90), true) -- yes, +200
	...
	Celer:spawn_occluder('7x8', Vector3(-790, 3450, 40), Rotation(0, 0, 0))
	Celer:spawn_occluder('7x8', Vector3(-790, 3450, 200), Rotation(0, 0, 0), true)
	...
	function WorldDefinition:make_unit(data, ...)
		...
		if name:find('units/payday2/architecture/mkp/') then
			return
		elseif not can_be_added_to_unit_group(name)
