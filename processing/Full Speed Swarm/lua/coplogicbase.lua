...
function CopLogicBase.on_attention_obj_identified(data, attention_u_key, attention_info)
	...
	if group then
		local t = group.fs_attention_obj_identified_t[attention_u_key]
		if not t or data.t - t > 0.1 then
			group.fs_attention_obj_identified_t[attention_u_key] = data.t
...
function CopLogicBase._get_logic_state_from_reaction(data, reaction)
	...
	local police_call = _is_loud or managers.groupai._state:chk_enemy_calling_in_area(managers.groupai._state:get_area_from_nav_seg_id(data.unit:movement():nav_tracker():nav_segment()), data.key)
...
function CopLogicBase._set_attention_obj(data, new_att_obj, new_reaction)
	...
	if new_att_obj then
		...
		if contact_chatter_time and new_crim_rec and data.t - new_crim_rec.det_t > contact_chatter_time then
			if new_reaction >= REACT_SHOOT and new_att_obj.verified and new_att_obj.is_person and data.char_tweak.chatter.contact and (data.unit:anim_data().idle or data.unit:anim_data().move) then
