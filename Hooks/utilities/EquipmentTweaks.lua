...
elseif string.lower(RequiredScript) == "lib/units/equipment/ecm_jammer/ecmjammerbase" then
	...
	function ECMJammerBase:setup(...)
		setup_original(self, ...)
		if  true and managers.groupai:state():whisper_mode() then
	...
	function ECMJammerBase:contour_interaction(...)
		if not (managers.groupai:state():whisper_mode() and  true) then
...
elseif string.lower(RequiredScript) == "lib/units/interactions/interactionext" then
	BaseInteractionExt.SHAPED_CHARGE_TIMEOUT =  0.25		--Timeout for 2 InteractKey pushes, to prevent accidents in stealth
	BaseInteractionExt.KEYCARD_DOORS_TIMEOUT =  0.25		--Timeout for 2 InteractKey pushes, to prevent accidents in hoxton breakout day 2
	...
	function BaseInteractionExt:interact_start(player, data, ...)
		...
		if true and managers.groupai:state():whisper_mode()
