...
if Network:is_client() then
	...
	function EnemyManager:on_enemy_died(dead_unit, damage_info)
		...
		if not self:is_corpse_disposal_enabled() then
			table_insert(attached_corpses, dead_unit)
		end
	end
	...
	DelayedCalls:Add('DelayedModFSS_corpselim', 0, function()
		local fs_original_enemymanager_corpselimit = EnemyManager.corpse_limit
		function EnemyManager:corpse_limit()
			for _, unit in ipairs(attached_corpses) do
			...
			attached_corpses = {}
			self._corpse_disposal_upd_interval = math.max(0.15, self._corpse_disposal_upd_interval)
			EnemyManager.corpse_limit = fs_original_enemymanager_corpselimit
			return fs_original_enemymanager_corpselimit(self)
		end
	end)
end

local fs_original_enemymanager_updcorpsedisposal = EnemyManager._upd_corpse_disposal
function EnemyManager:_upd_corpse_disposal()
	fs_original_enemymanager_updcorpsedisposal(self)

	local units = World:find_units('all', 0)
	if #units > 10 then
